...
#funcion para normalizar los subproductos DISCA 
normaliza_subprod <- function(x) {
  x %>%
    str_trim() %>%                      
    str_to_lower() %>%                  
...
# Agrego columna normalizada en ambas tablas
productos_disca <- productos_disca %>%
  mutate(SUB_NORM = normaliza_subprod(SUB_PRODUCTO_PRINCIPAL))
...
df_legales_tabla <- df_legales %>% 
  ...
  semi_join(productos_disca %>% select(SUB_NORM), by = "SUB_NORM")
